...
// ── Pest Strategy Pattern ────────────────────────────────────

abstract class PestStrategy {
  PredictionResult? calculate(
    FarmModel farm,
...
class ThripsStrategy implements PestStrategy {
  ...
  PredictionResult? calculate(
    ...
    if (weather.rainfall24h > 15.0) {
      return PredictionService.buildResult(
        ...
        [
          'Lakukan pemantauan 2 hari pasca hujan reda.',
          'Fokuskan pengamatan pada pucuk daun muda.',
        ],
      );
    }

    String detail =
        'Kondisi lingkungan saat ini sangat mendukung perkembangan Thrips karena:';
    ...
    if (farm.mulchType == MulchType.silver &&
        farm.cropStage == CropStage.seedling) {
      score -= 0.25;
      detail +=
...
class FruitFlyStrategy implements PestStrategy {
  ...
  PredictionResult? calculate(
    ...
    if (farm.cropStage != CropStage.fruiting &&
        farm.cropStage != CropStage.harvesting) {
      return null;
    }
    ...
    } else if (weather.temperature < 16.0) {
      return PredictionService.buildResult(
        'Lalat Buah',
        0.1,
        'Suhu dingin hambat aktivitas.',
        'Suhu <16°C membuat lalat pasif.',
        ['Pantau saja.'],
      );
    }
    ...
    final bool hasFruitHost = nearbyPlants.any(
      (p) => ['Mangga', 'Jambu', 'Jeruk', 'Pepaya'].contains(p),
    );
    if (hasFruitHost) {
      score += 0.15;
      detail +=
...
class AnthracnoseStrategy implements PestStrategy {
  ...
  PredictionResult? calculate(
    ...
    final bool isWet = weather.wetHours > 4 || weather.humidity > 85.0;
    final bool isWarm =
        weather.temperature >= 20.0 && weather.temperature <= 30.0;
    ...
    if (isWet && isWarm) {
      ...
      return PredictionService.buildResult(
        'Antraknosa (Patek)',
        0.1,
        'Kondisi kering menghambat jamur.',
        'Spora membutuhkan lapisan air untuk berkecambah.',
        ['Jaga drainase.'],
      );
    }
...
class AphidStrategy implements PestStrategy {
  ...
  PredictionResult? calculate(
    ...
    if (farm.cropStage == CropStage.seedling ||
        farm.cropStage == CropStage.vegetative) {
      score += 0.35;
      detail +=
...
class PredictionService {
  ...
  List<PredictionResult> analyzeRisk(
    FarmModel? farm,
    WeatherModel weather,
    List<String> nearbyPlants,
  ) {
    ...
    for (final strategy in _strategies) {
      ...
      if (result != null) {
        if (farm.lastPesticideTime.contains('< 3 hari')) {
          final double newPercentage = result.percentage * 0.2;
          result = buildResult(
            result.pestName,
            newPercentage,
  ...
  /// Factory for building a [PredictionResult] with clamped score.
  static PredictionResult buildResult(
    String name,
    double score,
    String shortReason,
    String detailReason,
  ...
  static RiskLevel _getLevel(double score) {
